#!/usr/bin/env python3
"""Daily wave driver for experiment 003, run by launchd (io.spyglasses.aeo-exp003).
...
One `submit --intent headphones` covers all four headphone panels (hum,
spy_a, spy_b, neu — 257 prompts); the coffee floor (40 prompts, wave 1 only)
is submitted manually alongside wave 1 per spec §8 and never touched here.

Everything else (idempotent submission, cost cap, task_get-only polling) is
inherited from scripts/llm_scraper.py. Logs append to data/raw/wave_runs.log
via the launchd plist redirection.
"""
...
UV = "/usr/local/bin/uv"
LABEL = "io.spyglasses.aeo-exp003"
...
def notify(msg: str) -> None:
    try:
        subprocess.run(
            ["osascript", "-e",
             f'display notification "{msg}" with title "AEO experiment 003"'],
...
def ledger_state() -> tuple[int, bool, bool]:
    ...
    def local_date(iso: str) -> str:
        ...
        # concludes "already submitted today" and skips its wave (002's
        # wave-3 bug).
...
def main() -> None:
    ...
    wave = max_wave + 1
    log(f"submitting wave {wave} (257 headphone-panel prompts: hum+spy_a+spy_b+neu)")
    rc = cli(
