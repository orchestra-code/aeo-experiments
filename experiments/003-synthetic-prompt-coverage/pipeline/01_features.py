...
One row per collected task: arm (from the item_id prefix), extracted brands
(ordered), cited URLs/domains (from ``sources[]`` only — ``search_results[]``
are SERP extras, not citations; spec §2 Audit B), grounding-query token set,
answer text (interim only, never published), word count, model version.
Extraction code is identical to experiment 002's so shares are comparable.
...
The synthetic generator plants a known anchor bias (spy panels over-mention
their own anchor) and a known coffee floor so the gate and the H3 machinery
can be exercised.
"""
...
#: Planted effect sizes for the dry run: spy panels over-mention their own
#: anchor by this much (H3 must detect it), neu draws from the hum
#: distribution (H1 should come back near-exchangeable).
SYNTH_ANCHOR_BOOST = 3.0
...
def build_synthetic() -> pd.DataFrame:
    ...
    for r in prompts.itertuples():
        w = weights[r.intent].copy()
        anchor = ANCHORS.get(r.arm)
        if anchor:
            w[pool[r.intent].index(anchor)] *= SYNTH_ANCHOR_BOOST
        w = w / w.sum()
        profile = list(
            rng.choice(pool[r.intent], size=min(6, len(pool[r.intent])), replace=False, p=w)
        )
        n_waves = CONTRAST_WAVES if r.arm == "coffee" else WAVES
        for wave in range(1, n_waves + 1):
            ...
            result = {
                ...
                "fan_out_queries": [
                    f"best {r.intent} for travel", f"{keep[0]} review" if keep else "top picks",
                ],
...
def main() -> None:
    ...
    print(df.groupby(["arm", "wave"]).size().to_string())
