...
Seeded verbatim from experiment 002's post-curation lexicon (its Audit-D
validated state, incl. the wave-1-mined coffee agencies) so 003's brand shares
are directly comparable to 002's. Re-curated after 003's wave 1 via
mine_candidates() + human review — the current ChatGPT model may recommend
brands 002's model never surfaced (extraction code, not a hypothesis;
refinements are allowed post-freeze and logged in spec "Deviations"). Audit D
re-validates precision/recall on a 30-response manual spot-check before the
lexicon is frozen for analysis.
"""
