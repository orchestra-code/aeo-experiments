...
Sequence (house rule: positive control FIRST — exit 1 if it fails):
  ...
  H3     anchor bias: own-anchor share inflation, difference-in-differences
         across the two spy panels (directional, CI excluding 0)
...
def h2_h3_panel_shares(df: pd.DataFrame, rows: list, lines: list) -> None:
    ...
    # Anchor brands enter the matrices regardless of basket membership so H3
    # is defined even if an anchor falls under the floor in the human panel.
    ...
    for arm, anchor in ANCHORS.items():
        stat_fns[f"H3_own_vs_hum:{arm}"] = (
            lambda s, arm=arm, anchor=anchor: float(
    ...
    for name in (*[f"H3_own_vs_hum:{a}" for a in ANCHORS], "H3_did"):
        obs, lo, hi = results[name]
        significant = lo > 0
        rows.append({"test": name, "family": "brand_share",
                     "contrast": name.removeprefix("H3_"), "estimate": obs,
                     "lo": lo, "hi": hi, "directional_significant": significant})
        lines.append(
            f"H3 ({name.removeprefix('H3_')}): {obs:+.3f} [{lo:+.3f}, {hi:+.3f}] "
            f"-> {'anchor bias detected' if significant else 'no directional evidence'}"
        )
...
def main() -> None:
    ...
    rows: list[dict] = []
    lines: list[str] = [
        f"# Experiment 003 — model results{' (SYNTHETIC)' if synthetic else ''}",
    ...
    if passed:
        ...
        h2_h3_panel_shares(df, rows, lines)
        lines.append("")
