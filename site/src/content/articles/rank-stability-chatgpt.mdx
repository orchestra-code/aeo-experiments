...
- **The instability is in the membership, not the ranking.** Between runs of the same prompt, roughly a quarter of the recommended-brand set turns over (set overlap 0.74, from the parent study) — but among the brands that survive both runs, relative order holds at a mean Kendall's tau of **+0.64** [+0.62, +0.67]. In 46% of pairs the shared brands come back in *exactly* the same order; full reversals occur in under 1%.
...
The mechanism is no mystery in hindsight: the order of first mention tracks the same popularity gradient that drives membership. Sony doesn't just appear in 90% of answers; it *opens* 60% of them. Order inherits stability from the market structure the answers reflect.
...
The two statistics are designed to be complements: the parent study's set overlap measures whether a brand *survives* from one answer to the next, and tau measures order *among the survivors*. The same reader, on seeing the numbers, phrased the combination more sharply than we had: **the instability isn't in the ranking, it's in the membership.** Roughly a quarter of the set turns over between same-prompt runs; the ordering of what remains mostly holds. That's a more precise claim than either "rank is useless" or "rank is stable," and as far as any of us can tell, it hadn't been made with numbers attached before.
...
- **Read ordered AI answers as meaningful, but read the top slot as weather.** The middle of the ordering carries real signal about how the model sorts the market. The lead position, across the phrasings your customers actually use, is close to a market-share-weighted coin flip.
