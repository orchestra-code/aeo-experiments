...
- **Order, conditional on membership, is not chaos.** When the same prompt is re-run on different days, the brands both answers share appear in the same relative order at a mean Kendall's tau of **+0.64** [+0.62, +0.67] — and in 46% of pairs, in *exactly* the same order. Full reversals occur in under 1% of pairs.
...
- **Tau is conditional on shared membership (3+ brands).** It says nothing about the brands that churned out of the pair — that's the main study's territory. The two statistics are complements, not substitutes; use both.
